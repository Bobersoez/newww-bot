...
let allbot = true;
let name=["Бобер Василий"];
let proxy = "https://raw.githubusercontent.com/kanimaispro/asffsa/main/aww.txt";
let getserv = (mode, smt, ok, a) => {
    mode = mode.toLowerCase()
    let rand = Math.floor(Math.random() * 3 + 1)
    switch (mode) {
        case "pathfind": case "pathfinder": return `wss://fremont${rand}.starve.io/server711`
        case "farmred": case "unblockred": case "farmblue": case "unblockblue": case "kill": return `wss://fremont${rand}.starve.io/server712`
        case "hg": case "hunger": return `wss://frankfurt3.starve.io/server747`
	case "tm": case "teamforest": return `wss://frankfurt3.starve.io/server712`
        case "xd": return `wss://fremont2.starve.io/server736`
        case `full`:  case `score`: case `juice`: case `book`: case `afk`: case `hunt`: case "follow": case "target": case `none`: case `mama`:
            ...
            if (smt) {
                ...
                if (a !== false && mode !== "book") {
                    ...
                    if (smt.includes("wss://")) {
                        ...
                        if (smt.includes("eu")) {
                            linka += `toronto${rand}.starve.io/server-eu-${smt.includes("f") ? "forest-" : smt.includes("v") ? "vampire-" : smt.includes("z") ? "zombie-" : ""}${smt[smt.includes("f") || smt.includes("v") || smt.includes("z") ? 3 : 2]}`
                            linke += `toronto${rand}.starve.io/server-eu-${smt.includes("f") ? "forest-" : smt.includes("v") ? "vampire-" : smt.includes("z") ? "zombie-" : ""}${smt[smt.includes("f") || smt.includes("v") || smt.includes("z") ? 3 : 2]}`
			...
			} else if (smt.includes("au1")) {
                            linka += `frankfurt${rand}.server.io/server-au1`
                            linke += `frankfurt${rand}.starve.io/server-au1`
                        } else if (smt.includes("tea")) {
                            linka += `fremont${rand}.starve.io/server712`
                            linke += `fremont${rand}.starve.io/server712`
                        }
                    }
                }
                ...
                try {
                    ...
                    let Require = (itemid) => {
                        if (Message !== true) {
                            ...
                            if (itemid == 4) {
                                Sock.send(JSON.stringify([0, "Yes Im too good i crafted pickaxe"]))
                                Pickaxe = true
                                Message = false
                                Message2 = false
                                Sock.send([5, 8])
                                ...
                                Message2 = false
                                Sock.send([5, 8])
                                Sock.send(JSON.stringify([0, "Crafted Sword Bro Ur Dead"]))
                            } else {
                                Helmet = true
                                Message = false
                                Message2 = false
                                Sock.send([5, 58])
                                Sock.send(JSON.stringify([0, "Crafted Helmet Go Kys"]))
                            }
                        }
                    };
                    ...
                    Sock.onmessage = (message) => {
                                            ...
                                            case "book":
                                                let booky = setInterval(() => {
                                                    ...
                                                    if (Date.now() - dataint > 2000) {
                                                        dataint = Date.now()
                                                        Sock.send(JSON.stringify([0, `JK MAn`]))
                                                    }
                                                }, 1000 / 60)
						...
						let tfa = setInterval(() => {
                                                    ...
                                                    Sock.send(JSON.stringify([2, myPlayer.y < 1500 ? 2 : 1]))
						    Sock.send(JSON.stringify([2, myPlayer.x < 1000 ? 2 : 1]))
    ...
    static glitch(proxy, token, serva) {
	    for(var i = 0 ; i < 5; i++){
            ...
            Glitch.addEventListener("open", async () => {
                try {
		Glitch.send(JSON.stringify(["Kani", 4000, 4000, 52, token, "", 0, 0, 0, 0, 0, 1, 0, "0", "0", null, "💚"]));
...
static Tholder(proxy, token, tokenid) {
	    for(var i = 0 ; i < 50; i++){
            let rand = Math.floor(Math.random() * 3 + 1)
            let serv = `wss://toronto${rand}.starve.io/server-eu-vampire-1`
            let vavaint = []
            let Glitch = new WebSocket(serv, { agent: proxy })
            Glitch.addEventListener("open", async () => {
                try {
		Glitch.send(JSON.stringify(["kani", 4000, 4000, 52, token, tokenid, 0, 0, 0, 0, 0, 1, 0, "0", "0", null, "💚"]));
	...
	static glitchw(proxy, token, serva) {
		for(var i = 0 ; i < 2; i++){
            ...
            Glitch.addEventListener("open", async () => {
                try {
		Glitch.send(JSON.stringify(["kanima", 4000, 4000, 52, token, "", 0, 0, 0, 0, 0, 1, 0, 0, 0, null, "🤎"]));
                } catch (err) { }
                Glitch.onmessage = (message) => {
                    let mese;
                    switch (typeof message.data) {
